...
while(1):
    ...
    if person_flag == False:
        r,cp_frame = pd.personDetect(cp_frame,log)
        person_flag = tc.keep(r,cp_frame)
        cv2.imshow('detect:person',cp_frame)
        if person_flag == True:
            log.write("detect person\n")
        else:
            log.write("cannot detect person\n")

    if person_flag == True:
        r,cp_frame = mj.judge_marker(md,cp_frame,log)
        marker_flag = tc.keep(r,cp_frame)
        cv2.imshow('detect:marker',cp_frame)
        if marker_flag == True:
