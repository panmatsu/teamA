...
class TimeCount:
    ...
    def personKeep(self,pd,img,log):
        
        ret,frame = pd.personDetect(img,log)
        cv2.imshow('detect:person',frame)

        if ret == True and self.start_time == 0.0:
            self.start_time = time.time()
            log.write("start time measurement(person)\n")
        if ret == True and self.start_time != 0.0:
            self.frame_count += 1
        ...
        if self.start_time != 0 and time.time()-self.start_time > self.t and self.frame_count > self.f:
            self.start_time = 0.0
            self.frame_count = 0
            log.write("person key open\n")
            return True

        elif self.start_time != 0 and time.time()-self.start_time > self.t and self.frame_count > self.f:
            self.start_time = 0.0
            self.frame_count = 0
            log.write("person key close\n")
            return False

        else:
            return False

    def markerKeep(self,mj,md,img,log):
        
        ret,frame = mj.judge_marker(md,img,log)
        cv2.imshow('detect:marker',frame)
